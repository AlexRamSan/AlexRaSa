// Vista alzada, sin callouts. Soporta escalones en SL.
// Hélices: Z exacto, largo = CL, arrancan D/2 izq y terminan D/2 der, inclinación contraria.
// Rectángulos del modelo: CL (width=D, height=CL), SL (width variable por escalones), OHL (width=AD, height=OHL).
...
export function renderSVG(svg, s){
  ...
  // Texto de la cota en el LADO IZQUIERDO de la línea
  ...
  // Consistencia: TL controla escala. Si no viene, usar suma.
  ...
  const WIDTH_BODY  = s.D  * scale;   // ancho por D
  const WIDTH_SHANK = s.AD * scale;   // ancho por AD
  ...
  // ---------- HÉLICE / FILOS sobre CL ----------
  {
    ...
    const spanStart = leftBody - halfD;            // D/2 izq
    const spanEnd   = leftBody + WIDTH_BODY + halfD; // D/2 der
  ...
  // ---------- SL con ESCALONES ----------
  // Se dibuja una serie de tramos {d,l} centrados, hasta consumir SL.
  // El sobrante, si queda, continúa con el último diámetro.
  {
    const steps = Array.isArray(s.steps) ? s.steps : [];
    let rem = s.SL;         // largo total disponible en SL
    let curY = y;
    let lastD = s.D;

    for(const st of steps){
      if(rem <= 0) break;
      const segL = Math.max(0, Math.min(st.l || 0, rem));  // mm
      if(segL <= 0) continue;
      const w = (st.d || lastD) * scale;
      const left = cx - w/2;
      rect(left, curY, w, segL*scale, fillSL, strokeAll);
      curY += segL*scale;
      rem  -= segL;
      lastD = st.d || lastD;
    }
    ...
    if(rem > 0){
      const w = (lastD) * scale;  // continúa con el último diámetro
      const left = cx - w/2;
      rect(left, curY, w, rem*scale, fillSL, strokeAll);
      curY += rem*scale;
      rem = 0;
    }

    // avanzar Y hasta fin de SL
    y = top + (s.CL + s.SL)*scale;
  }
  ...
  // Izquierda: TL con TEXTO A LA IZQUIERDA
  dimVLeft(xLeft, top, top + TL*scale, `TL ${fmt(TL, s.unit)}`);
