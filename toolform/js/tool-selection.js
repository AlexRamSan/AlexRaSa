...
// Integrar con tu estilo existente. Fetch relativo a data/cutting-data.json
(() => {
  // Configuración base (se puede sobrescribir si data/cutting-data.json está presente)
  let VC_TABLE = {
  ...
  function recommendTooling(materialCode) {
    if (materialCode === 'al') return { toolMaterial: 'Carburo (pulido)', coating: 'Ninguno o pulido/AlTiB', geometry: 'hélice alta, radio pequeño' };
  ...
  // Render del UI en #tool-selection-root
  ...
  <div class="dark-card">
    ...
    <form id="toolForm" autocomplete="off">
      ...
      <pre id="jsonOut" class="mt-3 p-3 bg-slate-800 rounded text-xs overflow-auto" style="max-height:260px"></pre>
    </form>
  ...
  function renderVendors() {
    ...
    `).join('');
    out.querySelectorAll('button[data-oid]').forEach(b => b.addEventListener('click', (e) => {
      const id = Number(e.currentTarget.getAttribute('data-oid')); orderFromVendor(id);
    }));
    out.querySelectorAll('button[data-rid]').forEach(b => b.addEventListener('click', (e) => {
  ...
  document.getElementById('calcBtn').addEventListener('click', ()=> {
    ...
    const out = {...form, recommendation: rec.recommendation, metrics:{vc:rec.vc, rpm:rec.rpm, fz:rec.fz, feed:rec.feed, mrr:rec.mrr}, heuristics: rec.heuristics, ae_mm: rec.ae_mm};
    document.getElementById('jsonOut').textContent = JSON.stringify(out,null,2);
    if (window.integrateWithRpmCalculator) {
      try { window.integrateWithRpmCalculator(out); } catch (e) { console.warn('integrateWithRpmCalculator falló', e); }
    }
  });
  ...
  // Submit (POST to your API). Cambia ruta si es necesario.
  document.getElementById('toolForm').addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch('/api/tool-selection', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
  ...
  // Exponer integrador global (para unir con tu calculadora RPM)
  window.integrateWithRpmCalculator = window.integrateWithRpmCalculator || function(data){
    ...
    // Reemplaza con tu función que autollenará la calculadora RPM existente.
  };

  // form helpers restantes
  function readForm() {
    return {
      machine: machineTypeEl.value,
      units: unitsEl.value,
